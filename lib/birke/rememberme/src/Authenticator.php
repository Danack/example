...
class Authenticator
{
     ...
     * @return [LoginResult, array $user_data]
     */
    public function login()
    {
        ...
        if (!$cookieValue) {
            return [LoginResult::newNoCookieResult(), null];
        }

        $triplet = Triplet::fromString($cookieValue);

        if (!$triplet->isValid()) {
            return [LoginResult::newManipulationResult(), null];
        }
        ...
        switch ($tripletLookupResult) {
            case Storage\StorageInterface::TRIPLET_FOUND:
                ...
                return [LoginResult::newSuccessResult($triplet->getCredential()), $user_data];
            ...
            case Storage\StorageInterface::TRIPLET_INVALID:
                ...
                return [LoginResult::newManipulationResult(), null];
            default:
                return [LoginResult::newExpiredResult(), null];
        }
    }
     ...
     * @return $this
     */
    public function createCookie($credential, array $user_data)
    {
        ...
        $this->storage->storeTriplet($credential, $newToken.$this->salt, $newPersistentToken.$this->salt, $expire, $user_data);
